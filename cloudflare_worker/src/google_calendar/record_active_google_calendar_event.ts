...
import { saveActiveGoogleCalendarEventMapping } from "./google_calendar_state_database";

export async function recordActiveGoogleCalendarEvent(
  request: Request,
  ...
  await saveActiveGoogleCalendarEventMapping(
  ...
  return createJsonResponse({ recorded: true, google_event_id: googleEventId }, 200);
}
