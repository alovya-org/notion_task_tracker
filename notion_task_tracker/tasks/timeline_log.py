...
def parse_timeline_entries_from_fetched_task_page_content(fetched_page_content: str) -> list[dict[str, str]]:
    timeline_content = _extract_timeline_log_content_from_fetched_task_page_content(fetched_page_content)
    if timeline_content is None:
        return []

    timeline_entries = []
    seen_entry_dates = set()
    for heading in _parse_markdown_headings_from_content(timeline_content):
        entry_date = _parse_entry_date_from_timeline_heading(heading)
...
def check_fetched_task_page_has_usable_timeline_log(
    ...
    return _extract_timeline_log_content_from_fetched_task_page_content(fetched_page_content) is not None and bool(timeline_entries)


def render_initialised_task_timeline_markdown(
    entry_date: str,
    timeline_section_markdown: str,
    fetched_page_content: str,
) -> str:
    del entry_date
    existing_body_content = extract_body_content_to_subsume_under_initial_timeline_date(fetched_page_content)
...
def build_timeline_entry_for_date(entry_date: str) -> dict[str, str]:
    return {
        "entry_date": entry_date,
...
def extract_body_content_to_subsume_under_initial_timeline_date(fetched_page_content: str) -> str:
    body_content = _extract_body_content_from_fetched_task_page_content(fetched_page_content)
    existing_timeline_content = _extract_timeline_log_content_from_body_content(body_content)
    if existing_timeline_content is None:
        return body_content.strip()

    return _extract_content_below_first_markdown_heading(existing_timeline_content).strip()


def _extract_body_content_from_fetched_task_page_content(fetched_page_content: str) -> str:
...
def _extract_timeline_log_content_from_fetched_task_page_content(fetched_page_content: str) -> str | None:
    return _extract_timeline_log_content_from_body_content(_extract_body_content_from_fetched_task_page_content(fetched_page_content))


def _extract_timeline_log_content_from_body_content(body_content: str) -> str | None:
...
def _extract_content_below_first_markdown_heading(content: str) -> str:
...
def _parse_markdown_headings_from_content(content: str) -> list[str]:
    headings = []
...
def _parse_entry_date_from_timeline_heading(heading: str) -> str | None:
